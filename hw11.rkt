...
(define (pipe-image pipe tile-size pipe-width filled?)
  (local
    [(define (pipe-image/custom-color pipe tile-size pipe-width bg-color pipe-color dir)
       (local
         [(define mid-left-value (quotient (- tile-size pipe-width) 2))
          ...
          (define pipe-sub-imgs
            (if (and filled? (pipe-top pipe) (pipe-bottom pipe) (pipe-left pipe) (pipe-right pipe)) 
                (if (or (string=? "left" dir) (string=? "right" dir))
                    (append ;; left-right
                     (list (vertical-pipe "black") (vertical-pipe "black"))
                     (map (lambda (kind) (kind "green"))
                          (list horizontal-pipe horizontal-pipe)))
                    
                    (append ;; up-down
                     (map (lambda (kind) (kind "green"))
                          (list vertical-pipe vertical-pipe))
                     (list (horizontal-pipe "black") (horizontal-pipe "black"))))
         ...
         (foldl
          ...
          pipe-offsets-y
          pipe-sub-imgs)))]
    (pipe-image/custom-color pipe tile-size pipe-width "grey" (if filled? "green" "black") "up")))
...
(define (grid-image grid tile-size pipe-width goo-flow score)
  ;; Make a tile
    ...
    [(define TILE (rectangle tile-size tile-size "outline" "blue"))
     ...
     (define (place-pipe-on-grid grid-img x y pipe)
       (local
         ...
         (if (and (pipe? (pipe-at grid x y)) (my-member? (list x y)
                                  (map (lambda (e) (list (first e) (second e)))
                                       (if (and (not (empty? goo-flow)) (false? (first goo-flow)))
                                           (rest goo-flow)
                                           goo-flow))))
              (place-image
          (pipe-image (pipe-at grid x y)
                      tile-size
                      pipe-width
                      #t)
            img-x
...
;; given a gamestate, returns the current score
(define (get-score g)
  (* 50 (gamestate-score g)))


(define-struct gamestate [grid goo-flow incoming-pipes tile-size pipe-width score time])
;; A gamestate is
...
;; score: the number of points the player has
;; time: how long the game has been going on
...
(define (draw-interface gs)
  (local
    [(define incoming-pipes (gamestate-incoming-pipes gs))
     ...
     ;; incoming-pipes-img: incoming-pipe -> image
     ;; shows the list of incoming pipes on the side
     ...
     ;; score: Nunber -> image
     ;; given a score, displays it
     (define score-img
       (overlay/align "middle" "middle"
                 (text (string-append "Score:" (number->string (get-score gs))) 40 "white")
                 (rectangle (* tile-size (grid-dim (gamestate-grid gs))) 60 "solid" "black")))]
    (above (beside (draw-grid gs) incoming-pipes-img) score-img)))
...
(define (gamestate-propagate-goo gs)
  (make-gamestate (gamestate-grid gs)
                  ...
                  (gamestate-tile-size gs)
                  (gamestate-pipe-width gs)
                  (if (not (boolean? (first (gamestate-goo-flow gs))))
                      (add1 (gamestate-score gs))
                      (gamestate-score gs))
                  (gamestate-time gs)))
...
(define (gamestate-place-pipe-on-click gs x y event)
  (cond
      [(and (string=? "button-down" event)
            (not (empty? (gamestate-incoming-pipes gs)))
            (< (image-coor->grid-coor x (gamestate-tile-size gs)) (grid-dim (gamestate-grid gs)))
            (< (image-coor->grid-coor y (gamestate-tile-size gs)) (grid-dim (gamestate-grid gs))))
       (cond
         [(and (not (boolean? (pipe-at (gamestate-grid gs) (image-coor->grid-coor x (gamestate-tile-size gs)) (image-coor->grid-coor y (gamestate-tile-size gs)))))
               (my-member? (list (image-coor->grid-coor x (gamestate-tile-size gs)) (image-coor->grid-coor y (gamestate-tile-size gs)))
                           (map (lambda (e) (list (first e) (second e)))
                                (if (and (not (empty? (gamestate-goo-flow gs))) (false? (first (gamestate-goo-flow gs))))
                                    (rest (gamestate-goo-flow gs))
                                    (gamestate-goo-flow gs)))))
          gs]
         
         [(not (boolean? (pipe-at (gamestate-grid gs) (image-coor->grid-coor x (gamestate-tile-size gs)) (image-coor->grid-coor y (gamestate-tile-size gs)))))
          (make-gamestate (place-pipe (gamestate-grid gs)
                                      (first (gamestate-incoming-pipes gs)) (image-coor->grid-coor x (gamestate-tile-size gs)) (image-coor->grid-coor y (gamestate-tile-size gs)))
                          (gamestate-goo-flow gs)
                          (rest (gamestate-incoming-pipes gs))
                          (gamestate-tile-size gs)
                          (gamestate-pipe-width gs)
                          (sub1 (gamestate-score gs))
                          (gamestate-time gs))]
      
         [else (make-gamestate (place-pipe (gamestate-grid gs)
                                           (first (gamestate-incoming-pipes gs)) (image-coor->grid-coor x (gamestate-tile-size gs)) (image-coor->grid-coor y (gamestate-tile-size gs)))
                               (gamestate-goo-flow gs)
                       (rest (gamestate-incoming-pipes gs))
                       (gamestate-tile-size gs)
      ...
      [(and (string=? "button-down" event)
            (empty? (gamestate-incoming-pipes gs)))
       (gamestate-propagate-goo gs)]
      [else gs]))

;; count: Gamestate -> Gamestate
;; given a gamestate, countsdown the time left
(define (count g)
  (if (= 0 (gamestate-time g))
      (gamestate-propagate-goo (make-gamestate (gamestate-grid g)
                                               (gamestate-goo-flow g)
                                               (gamestate-incoming-pipes g)
                                               (gamestate-tile-size g)
                                               (gamestate-pipe-width g)
                                               (gamestate-score g)
                                               28))
     (make-gamestate (gamestate-grid g)
                     (gamestate-goo-flow g)
                     (gamestate-incoming-pipes g)
                     (gamestate-tile-size g)
                     (gamestate-pipe-width g)
                     (gamestate-score g)
                     (sub1 (gamestate-time g)))))
...
(define GS-1 (gamestate-init GRID-DIM 1 1 "right" TILE-SIZE PIPE-WIDTH DOUBLE-PASS-THROUGH-PIPES))
